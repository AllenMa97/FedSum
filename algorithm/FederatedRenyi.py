...
def q_c_p(device, y_hat_θ, s, c, p):
...
def G_θ_v(y_hat_θ, s, v, device):
    q_00 = q_c_p(device, y_hat_θ, s, 0, 0)
    q_01 = q_c_p(device, y_hat_θ, s, 0, 1)
    q_10 = q_c_p(device, y_hat_θ, s, 1, 0)
    q_11 = q_c_p(device, y_hat_θ, s, 1, 1)
    Q = torch.tensor([
        [q_00, q_01],
        [q_10, q_11]
    ]).to(device)
...
def argmax_v_LR(idxs_users, local_model_list, mask_s1_flag, training_dataset, client_dataset_list, r_hat_p0, r_hat_p1, device, γ_k_style):
...
def argmax_v_NN(idxs_users, local_model_list, mask_s1_flag, training_dataset, client_dataset_list, r_hat_p0, r_hat_p1, device, γ_k_style):
    training_dataset_size = len(training_dataset)
    ...
    for iter_t in range(algorithm_epoch_T):
        ...
        for i in range(num_clients_K):
            ...
            for batch_index, batch in enumerate(client_i_dataloader):
                ...
                regularization_term = lamda * G_θ_v(y_hat_θ, s, global_v, device).to(device)
        ...
        if (iter_t + 1) % communication_round_I == 0:
            ...
            theta_list = []
            γ_k = 1 / len(idxs_users)
            if "uniform_client" not in γ_k_style:
                γ_denominator = 0
                for id in idxs_users:
                    γ_denominator += len(client_dataset_list[id].indices)

            for id in idxs_users:
                selected_model = local_model_list[id]
                if "uniform_client" not in γ_k_style:
                    γ_numerator = len(client_dataset_list[id].indices)
            ...
            try:
                global_v = argmax_v_LR(idxs_users, local_model_list, mask_s1_flag, training_dataset, client_dataset_list, r_hat_p0, r_hat_p1, device, γ_k_style)
            except Exception:
                global_v = backup_v
    ...
    for iter_t in range(algorithm_epoch_T):
        ...
        for i in range(num_clients_K):
            ...
            for batch_index, batch in enumerate(client_i_dataloader):
                ...
                regularization_term = lamda * G_θ_v(y_hat_θ, s, global_v, device)
                regularization_term = torch.where(torch.isnan(regularization_term), torch.full_like(regularization_term, 0), regularization_term)
        ...
        if (iter_t + 1) % communication_round_I == 0:
            ...
            logger.info("********** Global v update **********")
            global_v = argmax_v_NN(idxs_users, local_model_list, mask_s1_flag, training_dataset, client_dataset_list, r_hat_p0, r_hat_p1, device, γ_k_style)
