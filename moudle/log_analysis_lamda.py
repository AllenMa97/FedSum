...
def log_to_table(log_path,
    ...
    for i in range(file_count):
        ...
        if Parameter_dict["FL_drop_rate"] != 0:
            continue
        # if Parameter_dict["algorithm_epoch_T"] != 120:
        #     continue
        # if Parameter_dict["communication_round_I"] != 30:
        #     continue
        if Parameter_dict["lamda"] in [32, 16, 8]:
            continue
...
def main(dataset_name, hypothesis, split_strategy):
    ...
    # log_to_table(log_path, split_strategy=split_strategy)
